...
interface ListingRow {
  id: string; title: string; category: string; condition: string; price: number; currency: string;
  user_id: string; is_sold: boolean; is_hidden: boolean; is_verified: boolean; view_count: number;
  verification_image_url: string; created_at: string; author_name?: string; brand?: string | null; model?: string | null;
}
...
async function fetchListingsPage(debouncedSearch: string, filter: string, page: number): Promise<ListingRow[]> {
  ...
  return (data || []).map((l: any) => ({
    ...l,
    is_sold: l.is_sold ?? false, is_hidden: l.is_hidden ?? false, is_verified: l.is_verified ?? false,
    view_count: l.view_count ?? 0, currency: l.currency ?? "TWD",
...
export default function CommunityMarketplace() {
  ...
  const toggleField = async (listing: ListingRow, field: "is_verified" | "is_hidden" | "is_sold") => {
    ...
    if (error) {
      ...
      const labels: Record<string, [string, string]> = {
        is_verified: ["已審核通過", "已取消審核"], is_hidden: ["已下架", "已恢復上架"], is_sold: ["已標記售出", "已取消售出"],
      };
  ...
  const conditionLabel: Record<string, string> = { new: "全新", like_new: "幾乎全新", good: "良好", fair: "尚可", poor: "較差" };
              ...
              <TableBody>
                {allListings.map(listing => (
                  <TableRow key={listing.id} className={listing.is_hidden ? "opacity-50" : ""}>
                    ...
                    <TableCell className="text-center"><Badge variant="secondary" className="text-xs">{conditionLabel[listing.condition] || listing.condition}</Badge></TableCell>
                    <TableCell className="text-center">
                      <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => setPreviewImage(listing.verification_image_url)}><Eye className="h-3.5 w-3.5" /></Button>
                    </TableCell>
                    <TableCell className="text-center"><Switch checked={listing.is_verified} onCheckedChange={() => toggleField(listing, "is_verified")} disabled={togglingIds.has(listing.id)} /></TableCell>
                    <TableCell className="text-center"><Switch checked={listing.is_hidden} onCheckedChange={() => toggleField(listing, "is_hidden")} disabled={togglingIds.has(listing.id)} /></TableCell>
                    <TableCell className="text-center"><Switch checked={listing.is_sold} onCheckedChange={() => toggleField(listing, "is_sold")} disabled={togglingIds.has(listing.id)} /></TableCell>
                    <TableCell className="text-xs text-muted-foreground whitespace-nowrap">{format(new Date(listing.created_at), "yyyy/MM/dd")}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
