...
interface RoleUser {
  ...
  avatar_url: string | null; role: AppRole; created_at: string;
}
  ...
  return (rolesRes.data || [])
    ...
    .map(r => {
      ...
      return {
        ...
        username: p?.username || "未知", display_name: p?.display_name || null, avatar_url: p?.avatar_url || null,
      };
    });
}
...
export default function MemberRoles() {
  ...
  useAdminPage("會員角色", "管理會員權限與角色");

  const { data: roleUsers = [], isLoading: loading } = useQuery({
    ...
    <>
      ...
      {loading ? (
        ...
        <div className="border rounded-xl overflow-hidden mb-8">
          <Table>
            <TableHeader><TableRow className="bg-muted/50"><TableHead>會員</TableHead><TableHead className="w-28">指派時間</TableHead><TableHead className="w-20 text-right">操作</TableHead></TableRow></TableHeader>
            <TableBody>{admins.map(renderUserRow)}</TableBody>
          </Table>
        </div>
      )}
      ...
      {loading ? null : moderators.length === 0 ? (
        ...
        <div className="border rounded-xl overflow-hidden">
          <Table>
            <TableHeader><TableRow className="bg-muted/50"><TableHead>會員</TableHead><TableHead className="w-28">指派時間</TableHead><TableHead className="w-20 text-right">操作</TableHead></TableRow></TableHeader>
            <TableBody>{moderators.map(renderUserRow)}</TableBody>
          </Table>
        </div>
      )}
