...
interface TrafficData {
  pvTrend: TrendPoint[]; uvTrend: TrendPoint[];
  topPages: RankItem[]; topReferrers: RankItem[];
  topCountries: RankItem[]; topLanguages: RankItem[];
  deviceBreakdown: RankItem[];
  totalPV: number; totalUV: number; todayPV: number; todayUV: number;
  bandwidth: BandwidthData | null;
}
interface ContentData {
  photoTrend: TrendPoint[]; topicTrend: TrendPoint[]; userTrend: TrendPoint[];
  topPhotos: TopPhoto[]; activeUsers: ActiveUser[];
}
...
function getDateRange(rangeMode: string, rangeDays: number, customFrom?: Date, customTo?: Date) {
...
async function fetchTrafficAndBandwidth(rangeMode: string, rangeDays: number, customFrom?: Date, customTo?: Date): Promise<TrafficData> {
  const { startDate, days } = getDateRange(rangeMode, rangeDays, customFrom, customTo);
  const todayStart = new Date(); todayStart.setHours(0, 0, 0, 0);
  ...
  const emptyTraffic: TrafficData = {
    pvTrend: [], uvTrend: [], topPages: [], topReferrers: [],
    topCountries: [], topLanguages: [], deviceBreakdown: [],
    totalPV: 0, totalUV: 0, todayPV: 0, todayUV: 0, bandwidth: null,
  };

  if (!views || views.length === 0) return emptyTraffic;

  const totalPV = views.length;
  const totalUV = new Set(views.map((v: any) => v.session_id)).size;
  const todayViews = views.filter((v: any) => new Date(v.created_at) >= todayStart);
  const todayPV = todayViews.length;
  const todayUV = new Set(todayViews.map((v: any) => v.session_id)).size;
  ...
  views.forEach((v: any) => {
    pageCount.set(v.page_path, (pageCount.get(v.page_path) || 0) + 1);
    ...
    if (w < 768) mobile++; else if (w < 1024) tablet++; else desktop++;
  ...
  for (let i = 0; i < days; i++) { const d = format(subDays(new Date(), days - 1 - i), "MM/dd"); dailyMap.set(d, { photoViews: 0, pageViews: 0 }); }
  views.forEach((v: any) => {
  ...
  return {
    ...
    topPages: [...pageCount.entries()].sort((a, b) => b[1] - a[1]).slice(0, 10).map(([name, count]) => ({ name: formatPageName(name), count })),
    ...
    topLanguages: [...langCount.entries()].sort((a, b) => b[1] - a[1]).slice(0, 6).map(([name, count]) => ({ name: formatLang(name), count })),
    deviceBreakdown: [{ name: "手機", count: mobile }, { name: "平板", count: tablet }, { name: "桌面", count: desktop }].filter(d => d.count > 0),
    totalPV, totalUV, todayPV, todayUV,
    bandwidth: {
...
async function fetchContentData(rangeMode: string, rangeDays: number, customFrom?: Date, customTo?: Date): Promise<ContentData> {
  const { startDate, days } = getDateRange(rangeMode, rangeDays, customFrom, customTo);
  ...
  allPhotoUsers?.forEach((p) => countMap.set(p.user_id, (countMap.get(p.user_id) || 0) + 1));
  const sorted = [...countMap.entries()].sort((a, b) => b[1] - a[1]).slice(0, 10);

  // Batch fetch profiles for both top photos and active users
  const allUserIds = [...new Set([...(topPhotoRows || []).map(p => p.user_id), ...sorted.map(([uid]) => uid)])];
  ...
  return {
    ...
    topPhotos: (topPhotoRows || []).map(p => ({
      ...p, view_count: p.view_count || 0, like_count: p.like_count || 0, average_rating: p.average_rating || 0,
      author_name: profileMap.get(p.user_id)?.display_name || profileMap.get(p.user_id)?.username || "未知",
    })),
    activeUsers: sorted.map(([uid, count]) => ({
      user_id: uid,
      username: profileMap.get(uid)?.username || "未知",
      display_name: profileMap.get(uid)?.display_name || profileMap.get(uid)?.username || "未知",
      photo_count: count,
    })),
  };
}
...
export default function AnalyticsDashboard() {
  ...
  // Query 1: Stats (not date-dependent)
  ...
  // Query 2: Traffic + bandwidth (date-dependent)
  const dateKey = rangeMode === "custom" ? `${customFrom?.toISOString()}-${customTo?.toISOString()}` : String(rangeDays);
  const { data: traffic, isLoading: trafficLoading } = useQuery({
    queryKey: ["admin-analytics-traffic", dateKey],
    queryFn: () => fetchTrafficAndBandwidth(rangeMode, rangeDays, customFrom, customTo),
    staleTime: STALE_TIME,
  });

  // Query 3: Content trends + rankings (date-dependent)
  const { data: content, isLoading: contentLoading } = useQuery({
    queryKey: ["admin-analytics-content", dateKey],
    queryFn: () => fetchContentData(rangeMode, rangeDays, customFrom, customTo),
    staleTime: STALE_TIME,
  });
  ...
  const bandwidth = traffic?.bandwidth ?? null;
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <HighlightCard icon={<Eye className="h-5 w-5" />} label="今日瀏覽" value={traffic?.todayPV || 0} sub={`${traffic?.todayUV || 0} 獨立訪客`} color="text-amber-600 bg-amber-500/10" />
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-3 mb-6">
        {[
          ...
          { label: "總瀏覽量", value: traffic?.totalPV || 0, icon: <Eye className="h-4 w-4" /> },
        ].map(s => (
          ...
          {trafficLoading && !traffic ? (
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <StatCard label={`${rangeLabel}瀏覽`} value={traffic?.totalPV || 0} icon={<Eye className="h-5 w-5" />} />
                <StatCard label={`${rangeLabel}訪客`} value={traffic?.totalUV || 0} icon={<Users className="h-5 w-5" />} />
                <StatCard label="今日瀏覽" value={traffic?.todayPV || 0} icon={<FileText className="h-5 w-5" />} />
                <StatCard label="今日訪客" value={traffic?.todayUV || 0} icon={<Globe className="h-5 w-5" />} />
              </div>

              <div className="grid lg:grid-cols-2 gap-6">
                <TrendChart title={`瀏覽量趨勢（${rangeLabel}）`} icon={<Eye className="h-4 w-4" />} data={traffic?.pvTrend || []} color="hsl(var(--primary))" name="PV" />
                <TrendChart title={`獨立訪客趨勢（${rangeLabel}）`} icon={<Users className="h-4 w-4" />} data={traffic?.uvTrend || []} color="#10b981" name="UV" />
              </div>

              <div className="grid lg:grid-cols-2 xl:grid-cols-4 gap-6">
                <RankCard title="熱門頁面" icon={<FileText className="h-4 w-4" />} items={traffic?.topPages || []} />
                <RankCard title="流量來源" icon={<Link2 className="h-4 w-4" />} items={traffic?.topReferrers || []} />
                <RankCard title="訪客國家/地區" icon={<MapPin className="h-4 w-4" />} items={traffic?.topCountries || []} />
                <Card>
                  <CardHeader><CardTitle className="text-base flex items-center gap-2"><Monitor className="h-4 w-4" /> 裝置分佈</CardTitle></CardHeader>
                  <CardContent>
                    {(traffic?.deviceBreakdown?.length || 0) > 0 ? (
                      <>
                        <ResponsiveContainer width="100%" height={180}>
                          <PieChart>
                            <Pie data={traffic!.deviceBreakdown} dataKey="count" nameKey="name" cx="50%" cy="50%" outerRadius={70} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}>
                              {traffic!.deviceBreakdown.map((_, i) => (<Cell key={i} fill={COLORS[i % COLORS.length]} />))}
                            </Pie>
                            <Tooltip />
                          </PieChart>
                        </ResponsiveContainer>
              ...
              {(traffic?.topLanguages?.length || 0) > 0 && (
                <Card>
                  ...
                  <CardContent>
                    <ResponsiveContainer width="100%" height={220}>
                      <BarChart data={traffic!.topLanguages} layout="vertical">
            ...
            <>
              <div className="grid lg:grid-cols-3 gap-6">
                <Card>
                  ...
                  <CardContent>
                    <ResponsiveContainer width="100%" height={220}>
                      <AreaChart data={content?.userTrend || []}>
                        <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
                ...
                <Card>
                  ...
                  <CardContent>
                    <ResponsiveContainer width="100%" height={220}>
                      <AreaChart data={content?.photoTrend || []}>
                        <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
                        <XAxis dataKey="date" tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }} />
                        <YAxis tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }} />
                        <Tooltip />
                ...
                <Card>
                  ...
                  <CardContent>
                    <ResponsiveContainer width="100%" height={220}>
                      <BarChart data={content?.topicTrend || []}>
                        <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
                        <XAxis dataKey="date" tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }} />
                        <YAxis tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }} />
                  ...
                  <Card>
                    <CardContent className="p-0">
                      <div className="divide-y divide-border">
                        ...
                        {(content?.topPhotos?.length || 0) === 0 && <div className="p-8 text-center text-muted-foreground">暫無資料</div>}
                      </div>
                    </CardContent>
                  </Card>
                </TabsContent>
                  ...
                  <Card>
                    <CardContent className="p-0">
                      <div className="divide-y divide-border">
                        ...
                        {(content?.activeUsers?.length || 0) === 0 && <div className="p-8 text-center text-muted-foreground">暫無資料</div>}
                      </div>
                    </CardContent>
                  </Card>
                </TabsContent>
          ...
          {trafficLoading && !traffic ? (
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                ...
                <QuotaCard
                  label="剩餘流量" limitGB={PLAN_BANDWIDTH_GB} usedMB={bandwidth.totalEstimatedMB} icon={<Activity className="h-5 w-5" />}
                  editing={editingBw} editValue={editBwValue} setEditValue={setEditBwValue} saving={savingQuota}
                  onEdit={() => { setEditBwValue(String(PLAN_BANDWIDTH_GB)); setEditingBw(true); }}
                  onCancel={() => setEditingBw(false)}
                  onSave={async () => { const v = Number(editBwValue); if (v > 0) { await saveQuotaSetting("plan_bandwidth_gb", String(v)); setEditingBw(false); } }}
                />
                <QuotaCard
                  label="剩餘儲存" limitGB={PLAN_STORAGE_GB} usedMB={(bandwidth.totalStoragePhotos * AVG_PHOTO_WEIGHT_KB) / 1024} icon={<HardDrive className="h-5 w-5" />}
                  editing={editingStorage} editValue={editStorageValue} setEditValue={setEditStorageValue} saving={savingQuota}
                  onEdit={() => { setEditStorageValue(String(PLAN_STORAGE_GB)); setEditingStorage(true); }}
                  onCancel={() => setEditingStorage(false)}
                  onSave={async () => { const v = Number(editStorageValue); if (v > 0) { await saveQuotaSetting("plan_storage_gb", String(v)); setEditingStorage(false); } }}
                />
              </div>
