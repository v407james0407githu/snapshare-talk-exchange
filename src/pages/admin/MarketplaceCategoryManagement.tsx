...
export default function MarketplaceCategoryManagement() {
  useAdminPage("市集分類管理", "管理二手市集的商品分類");
  ...
  const { data: categories = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("marketplace_categories" as any).select("*").order("sort_order");
  ...
  const saveMutation = useMutation({
    mutationFn: async (values: typeof emptyForm & { id?: string }) => {
      ...
      if (values.id) {
        const { error } = await supabase.from("marketplace_categories" as any).update(payload as any).eq("id", values.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("marketplace_categories" as any).insert(payload as any);
        if (error) throw error;
      }
    },
    onSuccess: () => {
