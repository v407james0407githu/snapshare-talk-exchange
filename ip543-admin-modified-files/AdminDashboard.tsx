...
async function fetchDashboardRecentReports(): Promise<Report[]> {
...
async function fetchDashboardHealthWarnings(get: (key: string, fallback?: string) => string) {
...
export default function AdminDashboard() {
  const { get, getNum, settings } = useSystemSettings();
  const [trendRange, setTrendRange] = useState(7);
  const queryClient = useQueryClient();

  const { data: statsData, isLoading: kpiLoading } = useQuery({
  ...
  const { data: healthWarnings = [], isLoading: healthLoading } = useQuery({
    queryKey: ["admin-dashboard-health", settings],
    queryFn: () => fetchDashboardHealthWarnings(get),
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <KpiCard title="今日新會員" value={stats.todayUsers.toLocaleString()} subtitle={`累計 ${stats.totalUsers.toLocaleString()}`} icon={<Users className="h-4 w-4" />} loading={kpiLoading} />
        <KpiCard title="今日新作品" value={stats.todayPhotos.toLocaleString()} subtitle={`累計 ${stats.totalPhotos.toLocaleString()}`} icon={<Image className="h-4 w-4" />} loading={kpiLoading} />
        <KpiCard title="今日新討論" value={stats.todayTopics.toLocaleString()} subtitle={`累計 ${stats.totalTopics.toLocaleString()}`} icon={<MessageSquare className="h-4 w-4" />} loading={kpiLoading} />
        <KpiCard title="今日新刊登" value={stats.todayListings.toLocaleString()} subtitle={`累計 ${stats.totalListings.toLocaleString()}`} icon={<Store className="h-4 w-4" />} loading={kpiLoading} />
        <KpiCard title="今日瀏覽量" value={stats.todayViews.toLocaleString()} icon={<Eye className="h-4 w-4" />} loading={kpiLoading} />
      </div>

      {/* Alerts Row */}
      <div className="grid lg:grid-cols-3 gap-4 mb-6">
        {stats.pendingReports > 0 && (
          <Link to="/admin/moderation/reports" className="flex items-center gap-3 p-4 rounded-xl border border-destructive/20 bg-destructive/5 hover:bg-destructive/10 transition-colors">
            <div className="p-2 rounded-lg bg-destructive/15 text-destructive"><Flag className="h-4 w-4" /></div>
        ...
        <div className="flex items-center gap-3 p-4 rounded-xl border border-border bg-card">
          <div className="p-2 rounded-lg bg-muted text-muted-foreground"><HardDrive className="h-4 w-4" /></div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium">儲存空間</p>
            <Progress value={storagePercent} className="h-1.5 mt-1.5" />
            <p className="text-xs text-muted-foreground mt-1">{(storageUsed / 1024).toFixed(1)} / {(storageQuota / 1024).toFixed(1)} GB</p>
          </div>
        </div>
        <div className="flex items-center gap-3 p-4 rounded-xl border border-border bg-card">
          <div className="p-2 rounded-lg bg-green-500/10 text-green-600"><Activity className="h-4 w-4" /></div>
          <div className="flex-1">
      ...
      {/* Health Warnings */}
      {(healthLoading || healthWarnings.length > 0) && (
        <div className="mb-6 p-4 rounded-xl border border-yellow-500/20 bg-yellow-500/5">
          ...
          {healthLoading ? (
            <div className="text-sm text-yellow-700/80">檢查中...</div>
          ) : (
            <ul className="space-y-1.5">
              {healthWarnings.map((w, i) => (
                <li key={i} className="text-sm flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-yellow-500 shrink-0" />
                  <Link to={w.link} className="text-yellow-700/80 hover:text-yellow-900 hover:underline transition-colors">{w.text}</Link>
            ...
            {[7, 14, 30].map((d) => (
              <button
                ...
                className={`px-2.5 py-1 text-xs rounded-md font-medium transition-colors ${
                  trendRange === d ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80"
                }`}
              >
                {d} 天
              </button>
        ...
        {trendLoading ? (
          ...
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={trendData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
              ...
              <Legend />
              <Line type="monotone" dataKey="新會員" stroke="hsl(var(--primary))" strokeWidth={2} dot={false} />
              <Line type="monotone" dataKey="新作品" stroke="hsl(142 76% 36%)" strokeWidth={2} dot={false} />
              <Line type="monotone" dataKey="瀏覽量" stroke="hsl(38 92% 50%)" strokeWidth={2} dot={false} />
            </LineChart>
          </ResponsiveContainer>
        )}
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="rounded-xl border border-border bg-card p-5">
          ...
          <div className="grid grid-cols-2 gap-2">
            {quickActions.map((a) => (
              <Link key={a.href} to={a.href} className="flex items-center gap-2.5 p-3 rounded-lg hover:bg-muted/50 transition-colors group">
                ...
                {a.badge ? <Badge variant="destructive" className="ml-auto text-xs">{a.badge}</Badge> : null}
              </Link>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          ...
          {reportsLoading ? (
            <div className="flex items-center justify-center py-8 text-sm text-muted-foreground">載入中...</div>
                ...
                <div key={r.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/30">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <Badge variant="outline" className="text-xs">{contentTypeLabels[r.content_type] || r.content_type}</Badge>
                      <span className="text-xs text-muted-foreground">{formatDistanceToNow(new Date(r.created_at), { addSuffix: true, locale: zhTW })}</span>
                    </div>
                    <p className="text-sm truncate">{r.reason}</p>
                  </div>
                  <div className="flex gap-1 ml-2 shrink-0">
                    <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => handleResolveReport(r.id, "resolved")}>
                      <CheckCircle className="h-3.5 w-3.5 mr-1" />處理
                    </Button>
                    <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => handleResolveReport(r.id, "dismissed")}>
                      <Ban className="h-3.5 w-3.5 mr-1" />駁回
                    </Button>
                  </div>
                </div>
              ))}
